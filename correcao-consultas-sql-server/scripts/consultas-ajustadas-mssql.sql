...
SELECT p.cdpro, -- Informação original (CodigoProduto)
       ...
       LTRIM(RTRIM(p.nmpro)) AS [TRIM], -- removendo espaços da coluna NomeProduto (o AS é dando um Alias, ou seja, um apelido para a coluna)
       REPLACE(p.nmpro, 'A', 'AA') AS [REPLACE], -- Substituido onde tiver A por AA no Tipo de Produto
       RIGHT('0000' + CAST(p.cdpro AS varchar(4)), 4) AS [LPAD],  -- Adiciona zeros a esquerda no código do produto até completar 9 dígitos - LPAD
       ...
       UPPER(LEFT(p.nmpro, 1)) + LOWER(SUBSTRING(p.nmpro, 2, LEN(p.nmpro))) AS [INICIAR_MAISCULO],
       LEN(p.nmpro) AS [LENGTH] -- Quantos caracteres tem o campo NomeProduto
...
-- Remover acentuação
DECLARE @input NVARCHAR(100) = N'TÊXTÔ ÂCCÊNTÚÂDÔ';

SELECT @input AS Original,
       @input COLLATE Latin1_General_BIN2 AS WithoutAccents;


-- Exemplo de como gerar um código sequencial para uma coluna
SELECT 
    ROW_NUMBER() OVER (ORDER BY nmpro) AS codigo,
...
SELECT 
    v.dtven as Data,
    DAY(v.dtven) as Dia,
    MONTH(v.dtven) as Mes,
    YEAR(v.dtven) as Ano,
    FORMAT(v.dtven, 'dd-MM-yyyy') as [Mes-Dia-Ano],
    FORMAT(v.dtven, 'yyyy-MM-dd') as [Ano-Mes-Dia],
    FORMAT(v.dtven, 'yyyyMMdd') as [AnoMesDia],
    GETDATE() as hora_atual,
